...
bool CubicProbing::doesExist(std::string id)
{
    int i = hash(id);
    int j = 1;
    while ((bankStorage1d[(i % 100000)]).balance != -1 && bankStorage1d[(i % 100000)].id != id && i < 800000)
    {
        i += (j * j * j);
        j += 1;
    }
    i = i % 100000;
    if (i > 800000 || bankStorage1d[i].balance == -2 || bankStorage1d[i].balance == -1)
    {
        return false;
    }
    else
...
bool CubicProbing::deleteAccount(std::string id)
{
    ...
    i = i % 100000;
    if (i > 800000 || bankStorage1d[i].balance < 0)
    {
        return false;
    }
    else
